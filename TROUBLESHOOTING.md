...
The sequence `start_value = 7,141,300` and the fact that 49,161 live features exist with IDs *below* that value together confirm the current SQL Server SEQUENCE object was **created or restarted at 7,141,300** — not that the sequence incremented naturally to reach it. Those older features were generated by a prior sequence mechanism (likely the `sde.sde_sequences` table before migration to a native SQL Server SEQUENCE object).
...
**This definitively rules out the millions-of-phantom-inserts hypothesis.** The TR1→TR714 ID gap is explained entirely by the sequence re-seed event.

**Action items generated from this step:**

- ✅ `sys.sequences` confirmed — sequence is healthy, no runaway behaviour.
- Determine *when and why* the SQL Server SEQUENCE object was created/restarted at 7,141,300. Check deployment records, geodatabase republish history, or SQL Server Default Trace (Section 4.2).
- Ask the DBA to re-examine the "no manual alteration" conclusion — a sequence drop + `CREATE SEQUENCE ... START WITH 7141300` may not have appeared as a manual "ALTER" in their original check.
- Proceed to **Section 2.2** to compare live vs. archive row counts (confirm delete+re-insert is still the mechanism for the affected features).
...
| 2 | Was the sequence `sdeadm.sectravid` ever manually altered? | **Needs re-examination.** DBA originally confirmed No, but `INFORMATION_SCHEMA.SEQUENCES` shows `start_value = 7,141,300` — meaning the SQL Server SEQUENCE object was either *created* at that value or *restarted* to it. IDs below 7,141,300 exist in the live table, indicating a prior sequence mechanism was in use. DBA should confirm (a) when the native SEQUENCE object was created/last restarted and (b) whether a sequence recreation (drop + CREATE) was performed — this would not appear as a manual "ALTER" in some audit logs. See Section 4.2. |
