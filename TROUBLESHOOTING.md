...
SELECT name, start_value, increment, current_value, minimum_value, maximum_value
FROM sys.sequences
...
#### 2.1 Findings (2026-03-06)

Query run against `[GISRW01].[INFORMATION_SCHEMA].[SEQUENCES]`. Result:
...
**Interpretation:** The `start_value` of 7,141,300 means the current native SQL Server SEQUENCE object was seeded at this value — not that the sequence incremented naturally to reach it. The 49,161 features with IDs *below* 7,141,300 were generated by the **prior SDE sequence mechanism** (`sde.sde_sequences` table-based), before the sequence was migrated or recreated as a native SQL Server SEQUENCE object. The current object has only generated ~759 IDs so far (7,141,300 → ~7,142,059).

**This revises the earlier hypothesis:** The ~6-million-number jump from `TR1xxxxxx` → `TR714xxxx` is *not* the result of millions of phantom inserts and rollbacks. It reflects the sequence being re-seeded at a much higher starting number during a geodatabase operation (republish, sequence recreation, or migration). The affected features (Kirk's ~30 features in the `TR7141xxx` range) were re-inserted *after* this re-seed, which is why they landed in the 7,141,800–7,141,900 range rather than continuing from the previous max ID.

**Action items generated from this step:**
- Run Option C (`sys.sequences`) to confirm the current live value of the sequence — this will also reveal whether `sde.sde_sequences` and the native sequence object are in sync.
...
- Ask the DBA to re-examine the "no manual alteration" conclusion in light of this evidence — a sequence recreation (drop + `CREATE SEQUENCE ... START WITH 7141300`) would not necessarily appear as a manual alteration in their original check.
- Proceed to Section 2.2 to compare live vs. archive row counts.
